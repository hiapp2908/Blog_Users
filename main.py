...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user_db:
            return f"<h1>Email doesn't exists. Register instead.</h1>" \
                   f"<br>" \
                   f"<a href='{url_for('register')}'>Register</a>"
        elif not check_password_hash(user_db.password, password):
            return f"<h1>Password Incorrect.</h1>" \
                   f"<br>" \
                   f"<a href='{url_for('login')}'>Try again</a>"
        else:
            login_user(user_db)
            return redirect(url_for('get_all_posts'))
